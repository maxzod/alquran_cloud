...
class Juz extends Equatable {
  ...
  factory Juz.fromMap(Map<String, dynamic> map) {
    return Juz(
      ...
      ayahs: ((map['ayahs'] as List<Map<String, dynamic>>?) ?? []).map<Aya>((x) => Aya.fromMap(x)).toList(),
      surahs: ((map['surahs'] as List<Map<String, dynamic>>?) ?? []).map<Surah>((x) => Surah.fromMap(x)).toList(),
